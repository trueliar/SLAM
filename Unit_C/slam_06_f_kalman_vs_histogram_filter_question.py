...
def kalman_filter_step(belief, control, measurement):
    ...
    a = 1             #constants and conversion factors set to default
    c = 1
    ...
    # Prediction.
    mu_t_bar = a * mu_t_1 + u_t
    sigma2_t_bar = a**2 * sigma2_t_1 + sigma2_R
    prediction = Density(mu_t_bar, sigma2_t_bar)

    K = c * sigma2_t_bar/(c**2 * sigma2_t_bar + sigma2_Q)
    mu = mu_t_bar + K * (Z - c * mu_t_bar)
    sigma2 = (1 - K * c) * sigma2_t_bar
