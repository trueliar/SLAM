...
class Particle:
    ...
    def wl_likelihood_of_correspondence(self, measurement,
        ...
        Ql = self.H_Ql_jacobian_and_measurement_covariance_for_landmark(landmark_number, Qt_measurement_covariance,
                                                                        scanner_displacement)[1]
        det_Ql = np.linalg.det(Ql)
        likelihood = (1 / (2 * pi * sqrt(det_Ql))) * exp(-0.5 * np.dot(delta_z.T, np.dot(np.linalg.inv(Ql), delta_z)))
    ...
    def update_landmark(self, landmark_number, measurement,
        ...
        K = np.dot(sigma_old, np.dot(H.T, Ql_inv))
        KH = np.dot(K, H)
    ...
    def update_particle(self, measurement, measurement_in_scanner_system,
        ...
        likelihoods = self.compute_correspondence_likelihoods(measurement, n, Qt_measurement_covariance,
                                                              scanner_displacement)
    ...
    def decrement_visible_landmark_counters(self, scanner_displacement):
        ...
        #   h_expected_measurement_for_landmark()
        for landmark in range(self.number_of_landmarks()):
